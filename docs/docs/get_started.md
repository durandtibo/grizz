...
To make the package as slim as possible, only the packages required to use `grizz` are
installed.
...
To install `grizz` from source, you can follow the steps below. First, you will need to
...
```shell
git clone git@github.com:durandtibo/grizz.git
```
...
```shell
conda activate grizz
```

This example uses `conda` to create a virtual environment, but you can use other tools or
configurations. Then, you should install the required package to use `grizz` with the
following command:
