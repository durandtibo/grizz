...
from grizz.lazy.ingestor import (
    CsvIngestor,
    ParquetFileIngestor,
    is_ingestor_config,
    setup_ingestor,
)
from grizz.lazy.ingestor.base import IngestorEqualityComparator
...
def test_is_ingestor_config_true() -> None:
    assert is_ingestor_config(
        {OBJECT_TARGET: "grizz.lazy.ingestor.CsvIngestor", "path": "/path/to/data.csv"}
    )
...
def test_setup_ingestor_object() -> None:
    ingestor = CsvIngestor(source="/path/to/data.csv")
...
def test_setup_ingestor_dict() -> None:
    assert isinstance(
        setup_ingestor(
            {OBJECT_TARGET: "grizz.lazy.ingestor.CsvIngestor", "source": "/path/to/data.csv"}
        ),
        CsvIngestor,
    )
