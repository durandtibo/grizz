...
from grizz.lazy.transformer import (
    DropNullRow,
    SqlTransformer,
    is_transformer_config,
    setup_transformer,
)
from grizz.lazy.transformer.base import TransformerEqualityComparator
...
def test_is_transformer_config_true() -> None:
    assert is_transformer_config({OBJECT_TARGET: "grizz.lazy.transformer.DropNullRow"})
...
def test_setup_transformer_dict() -> None:
    assert isinstance(
        setup_transformer({OBJECT_TARGET: "grizz.lazy.transformer.DropNullRow"}), DropNullRow
    )
...
TRANSFORMER_EQUAL = [
    pytest.param(
        ExamplePair(
            actual=DropNullRow(columns=["col1", "col4"]),
            expected=DropNullRow(columns=["col1", "col4"]),
        ),
        id="transformer",
    ),
    pytest.param(
        ExamplePair(
            actual=SqlTransformer(query=""),
            expected=SqlTransformer(query=""),
        ),
...
TRANSFORMER_NOT_EQUAL = [
    ...
    pytest.param(
        ExamplePair(
            actual=DropNullRow(columns=["col1", "col4"]),
            expected=DropNullRow(columns=["col1", "col2", "col4"]),
