...
from grizz.lazy.exporter import (
    CsvExporter,
    ParquetExporter,
    is_exporter_config,
    setup_exporter,
)
from grizz.lazy.exporter.base import ExporterEqualityComparator
...
def test_is_exporter_config_true() -> None:
    assert is_exporter_config(
        {OBJECT_TARGET: "grizz.lazy.exporter.CsvExporter", "path": "/path/to/data.csv"}
    )
