...
def test_min_horizontal_transformer_equal_true() -> None:
    assert MinHorizontal(columns=["col1", "col3"], out_col="out").equal(
        MinHorizontal(columns=["col1", "col3"], out_col="out")
    )


def test_min_horizontal_transformer_equal_false_different_columns() -> None:
    assert not MinHorizontal(columns=["col1", "col3"], out_col="out").equal(
        MinHorizontal(columns=["col1", "col2", "col3"], out_col="out")
    )


def test_min_horizontal_transformer_equal_false_different_out_col() -> None:
    assert not MinHorizontal(columns=["col1", "col3"], out_col="out").equal(
        MinHorizontal(columns=["col1", "col3"], out_col="col")
    )


def test_min_horizontal_transformer_equal_false_different_exclude_columns() -> None:
    assert not MinHorizontal(columns=["col1", "col3"], out_col="out").equal(
        MinHorizontal(columns=["col1", "col3"], out_col="out", exclude_columns=["col2"])
    )


def test_min_horizontal_transformer_equal_false_different_exist_policy() -> None:
    assert not MinHorizontal(columns=["col1", "col3"], out_col="out").equal(
        MinHorizontal(columns=["col1", "col3"], out_col="out", exist_policy="warn")
    )


def test_min_horizontal_transformer_equal_false_different_missing_policy() -> None:
    assert not MinHorizontal(columns=["col1", "col3"], out_col="out").equal(
        MinHorizontal(columns=["col1", "col3"], out_col="out", missing_policy="warn")
    )


def test_min_horizontal_transformer_equal_false_different_type() -> None:
    assert not MinHorizontal(columns=["col1", "col3"], out_col="out").equal(42)


def test_min_horizontal_transformer_get_args() -> None:
    assert objects_are_equal(
        MinHorizontal(columns=["col1", "col3"], out_col="out").get_args(),
        {
            "columns": ("col1", "col3"),
            "exclude_columns": (),
            "exist_policy": "raise",
