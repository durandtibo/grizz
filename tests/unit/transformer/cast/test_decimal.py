...
def test_decimal_cast_transformer_transform_exist_policy_ignore(
    ...
    assert_frame_equal(
        ...
        pl.DataFrame(
            ...
            schema={"col1": pl.Int64, "col2": pl.Decimal, "col3": pl.Int32, "col4": pl.String},
        ),
    )
...
###################################################
#     Tests for InplaceDecimalCastTransformer     #
###################################################
...
def test_inplace_decimal_cast_transformer_fit(
    ...
    with caplog.at_level(logging.INFO):
        transformer.fit(dataframe)
    assert caplog.messages[0].startswith(
        "Skipping 'InplaceDecimalCastTransformer.fit' as there are no parameters available to fit"
    )
