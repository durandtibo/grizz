r"""Contain the base class to implement a LazyFrame ingestor."""
...
class BaseIngestor(ABC, metaclass=AbstractFactory):
    r"""Define the base class to implement a LazyFrame ingestor.
    ...
    def ingest(self) -> pl.LazyFrame:
        r"""Ingest a LazyFrame.

        Returns:
            The ingested LazyFrame.
...
def is_ingestor_config(config: dict) -> bool:
    ...
    >>> from grizz.lazy.ingestor import is_ingestor_config
    >>> is_ingestor_config(
    ...     {"_target_": "grizz.lazy.ingestor.CsvIngestor", "source": "/path/to/data.csv"}
    ... )
    True
...
def setup_ingestor(
    ...
    >>> from grizz.lazy.ingestor import setup_ingestor
